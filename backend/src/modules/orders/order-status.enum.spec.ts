import { canTransitionOrderStatus, ORDER_STATUS } from './order-status.enum';
...
describe('order-status.enum', () => {
  ...
  describe('canTransitionOrderStatus', () => {
    it('allows valid transitions', () => {
      ...
      expect(
        canTransitionOrderStatus(OrderStatus.CONFIRMED, OrderStatus.PROCESSING),
      ).toBe(true);
      expect(
        canTransitionOrderStatus(OrderStatus.PROCESSING, OrderStatus.SHIPPED),
      ).toBe(true);
    });

    it('allows same status (no-op)', () => {
      expect(
        canTransitionOrderStatus(OrderStatus.PENDING, OrderStatus.PENDING),
      ).toBe(true);
    });

    it('disallows transitions from terminal states', () => {
      expect(
        canTransitionOrderStatus(OrderStatus.DELIVERED, OrderStatus.PENDING),
      ).toBe(false);
      expect(
