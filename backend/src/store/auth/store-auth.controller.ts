...
export class StoreAuthController {
  ...
  async googleAuth(@Req() req: express.Request, @Res() res: express.Response) {
    const state = Buffer.from(JSON.stringify({ context: 'store' })).toString('base64url');
  ...
  async googleCallback(@Req() req: express.Request, @Res() res: express.Response) {
    ...
    return res.redirect(`${appUrl}/login?token=${encodeURIComponent(result.accessToken)}`);
  }
  ...
  @Public()
  @Post('login')
  @ApiOperation({ summary: 'Login with email and password (store)', security: [] })
  ...
  @ApiOperation({ summary: 'Request password reset email (store)', security: [] })
