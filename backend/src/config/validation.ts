...
export function validateConfig(
  ...
  const jwtSecret =
    get(config, 'jwt.secret', 'JWT_SECRET') ?? 'change-me-in-production';
  const nodeEnv =
    (config.nodeEnv as string | undefined) ??
  ...
  if (nodeEnv === 'production') {
    ...
    if (!appUrl || appUrl.includes('example.com')) {
      errors.push(
        'APP_URL must be set to your main website URL in production (e.g. https://yoursite.com)',
      );
    }
    ...
    if (!adminUrl || adminUrl.includes('example.com')) {
      errors.push(
        'ADMIN_URL must be set to your admin portal URL in production (e.g. https://admin.yoursite.com)',
      );
    }

    const mailProvider = (
      get(config, 'mail.provider', 'MAIL_PROVIDER') ?? 'mock'
    ).toLowerCase();
    ...
    if (mailProvider === 'sendgrid') {
      ...
      if (!key)
        errors.push('SENDGRID_API_KEY is required when MAIL_PROVIDER=sendgrid');
    }

    const storageProvider = (
      get(config, 'storage.provider', 'STORAGE_PROVIDER') ?? 'cloudinary'
    ).toLowerCase();
